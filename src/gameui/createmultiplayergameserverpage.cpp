...
void CCreateMultiplayerGameServerPage::LoadMaps( const char *pszPathID )
{
	...
	while ( pszFilename )
	{
		...
		if ( !V_stricmp( ModInfo().GetGameName(), "Half-Life" ) && ( mapname[0] == 'c' || mapname[0] == 't') && mapname[2] == 'a' && mapname[1] >= '0' && mapname[1] <= '5' )
		{
			goto nextFile;
		}
...
void CCreateMultiplayerGameServerPage::LoadMapList()
{
	...
	if ( !V_stricmp(ModInfo().GetGameName(), "Half-Life" ) ) 
	{
...
bool CCreateMultiplayerGameServerPage::IsRandomMapSelected()
{
	...
	if (!V_stricmp( mapname, RANDOM_MAP ))
	{
		return true;
	}
	return false;
