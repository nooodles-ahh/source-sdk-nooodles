...
public:
	COptionsSubVideoAdvancedDlg( vgui::Panel *parent ) : BaseClass( parent , "OptionsSubVideoAdvancedDlg" )
	{
		...
		SetSize(
			PROPORTIONAL_VALUE( 482 ),
			PROPORTIONAL_VALUE( 358 )
		);
		SetSizeable( false );
		...
		m_pMatQueueMode = new ComboBox( this, "Multicore", 2, false );
		m_pMatQueueMode->AddItem( "#gameui_disabled", NULL );
		m_pMatQueueMode->AddItem( "#gameui_enabled", NULL );
	...
	virtual void ApplySchemeSettings( vgui::IScheme* pScheme )
	{
		...
		// Noodles; The year is 202X, outside of testing on old niche hardware
		// there is no real reason to have this accessible
	...
	void SetComboItemAsRecommended( vgui::ComboBox *combo, int iItem )
	{
		...
		wchar_t newText[512];
		V_snwprintf( newText, sizeof(newText) / sizeof(wchar_t), FMT_WS L" *", text );
	...
	void MarkDefaultSettingsAsRecommended()
	{
		...
		// assuming everyone is using at least a dual core machine in 2023
		SetComboItemAsRecommended( m_pMatQueueMode, 1 );
	...
	virtual void ApplyChanges()
	{
		...
		ApplyChangesToConVar( "mat_queue_mode", m_pMatQueueMode->GetActiveItem() );	
	...
	virtual void OnResetData()
	{
		...
		m_pMatQueueMode->ActivateItem( mat_queue_mode.GetInt() != 0 );
	...
	bool RequiresRestart()
	{
		if ( engine->IsInGame() )
		{
			...
			if ( pNetInfo && pNetInfo->IsLoopback() )
				return false;
...
COptionsSubVideo::COptionsSubVideo(vgui::Panel *parent) : PropertyPage(parent, NULL)
{
   ...
   m_pThirdPartyCredits->SetCommand(new KeyValues("OpenThirdPartyVideoCreditsDialog"));
	...
	m_pVRMode = new vgui::ComboBox( this, "VRMode", 0, false );
	m_pVRModeLabel = new vgui::Label( this, "VRModeLabel", "#GameUI_VRMode" );
...
void COptionsSubVideo::ApplySchemeSettings( vgui::IScheme* pScheme )
{
	BaseClass::ApplySchemeSettings( pScheme );
	LoadControlSettings( "Resource/OptionsSubVideo.res" );

	if( m_hOptionsSubVideoAdvancedDlg.Get() )
		m_hOptionsSubVideoAdvancedDlg.Get()->InvalidateLayout();
}
